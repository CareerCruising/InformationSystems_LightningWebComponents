...
export default class CallOutOppRenewal extends NavigationMixin(LightningElement) {
    @api recordId;
    @track ActionPanel = {
        Spinner: true,
        Alert: false,
    ...
    callCreateRenewal() {
        ...
        fetch('https://is.xello.world/api/Integrations/OpportunityCreateRenewal?OppId=' + this.recordId).then((response) => response.json())
            .then((jsonResponse) => {
                ...
                if(data.success) {
                    ...
                    SendEmail({toAddress: ['anthonya@xello.world'], subject: 'callOutOppRenewal Issue (OppSource: ' + this.recordId + ')', body: this.ActionPanel.Message})
                }   
            })       
            .catch((error) => {
                console.log(error);
                this.ActionPanel.Message = error;  
                SendEmail({toAddress: ['anthonya@xello.world'], subject: 'callOutOppRenewal Issue (OppSource: ' + this.recordId + ')', body: error})
            });
    }
